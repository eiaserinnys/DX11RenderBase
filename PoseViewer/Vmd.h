...
namespace vmd
{
	...
	static std::wstring sjis2utf8(const std::string& sjis)
	{
		...
		int utf16size = ::MultiByteToWideChar(
			932, MB_ERR_INVALID_CHARS, pSJIS, -1, 0, 0);
		if (utf16size != 0)
		{
			std::vector<wchar_t> utf16;
			utf16.resize(utf16size);
			if (::MultiByteToWideChar(
				932, 0, (LPCCH)pSJIS, -1, &utf16[0], utf16size) != 0)
			{
	...
	public:
		...
		static std::unique_ptr<VmdMotion> LoadFromStream(std::ifstream *stream)
		{
			...
			result->model_wname = sjis2utf8(result->model_name);
